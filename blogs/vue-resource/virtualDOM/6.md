...
> 在`_update`方法中可以发现, 真正处理`vnode`的地方是在`__patch__`方法中, 所以这里的核心应该是找到`__patch__`方法定义的位置

## Snabbdom 中的 vnode

[snabbdom 中的 Vnode 函数](/blogs/vue/vue-virtualDOM/4.html#vnode函数)

> Snabbdom 中的 VNode 比 Vue 中的 VNode 简单很多, 通过`vnode函数`返回了一个普通对象, 这个对象只有 6 个属性
>
> 而 Vue 中的 VNode 的属性复杂的多, 功能也比 snabbdom 复杂的多

![snabbdom](../images/snabbdomVnode.png)

## Snabbdom 中的 patch

[snabbdom 中的 patch 函数](/blogs/vue/vue-virtualDOM/5.html#init-函数)

> snabbdom 中 patch 函数是通过 init 函数返回的, 这里用到了`函数柯里化`
>
> 在 init 函数中, 先传入了`modules`和`domApi`这两个参数
>
> -   modules 是模块, 类似于插件机制, 在这里处理属性、样式、事件等
> -   也可以定义自己的模块
> -   domApi 是操作 dom 的方法
> -   init 中将这两个参数通过闭包去缓存, 然后返回一个`patch`方法, 后续调用`patch`函数时, 就不用关心模块和操作 DOM 的方法
> -   Vue 中机制类似

![snabbdom](../images/snabbdomPatch.png)

## vue 实例上的 patch 定义

> vue 实例中的 patch 方法定义在 `src/platform/web/runtime/index.js`中
>
> 该方法是与平台相关的, 在 web 平台的 runtime 下面
>
> 在这个文件中, 在设置 patch 之前, 首先判断当前环境是不是浏览器环境, 如果是浏览器环境, 则返回一个`patch`函数
>
> 如果不是浏览器环境, 直接返回一个空函数
...
> vue 的 patch 函数同样使用了 函数柯里化去缓存 modules 和 nodeOps
>
> 这个 patch 方法定义在`src/platform/web/runtime/patch`模块中, 如下
>
> 这个`patch`函数是通过`createPatchFunction`调用后传入`{ nodeOps, modules }`生成的, 这里的 modules 和 nodeOps 同样被缓存了
> 
> 这个函数同样是一个高阶函数(HOF), 也是函数柯里化
> 
> `nodeOps`从`src/platform/web/runtime/node-ops`中导入, 主要封装了一系列操作DOM的方法, 只是处理了一些其他内容(比如createElement中处理了`select`标签的`multiple`属性), 其实就类似`snabbdom`的init方法中缓存的`domApi`
> 
> [node-ops](/blogs/vue-resource/virtualDOM/6.html#node-ops)见下方定义处
> 
> `modules`主要是拼接了两个数组, 将下面两个数组直接拼接起来赋值给了 modules
>   + [platformModules](/blogs/vue-resource/virtualDOM/6.html#platformmodules)
>   + [baseModules](/blogs/vue-resource/virtualDOM/6.html#basemodules)
> 
> 当 `nodeOps`和`modules`都初始化完了, 则调用[createPatchFunction](/blogs/vue-resource/virtualDOM/6.html#createpatchfunction)去初始化patch函数
...
> 通过名字就可以发现, 内部的模块是和平台相关的, 这个模块也和`snabbdom`中的那个 modules 是一致的
> 
> 这些模块的作用其实就是操作 属性、 类名、 事件、 dom属性、 样式等
> 
> 不过其中 有一个 transition, 这个模块是 vue 中特有的, 用于处理过度动画
> 
> 这些模块中, 其实导出的就是一些VNode的生命周期钩子函数
...
> 与平台无关, 在`core/vdom/modules/index`中定义
> 
> 主要是处理ref和指令
...
> 这个函数中的代码非常多, 在最后返回了`patch`函数
> 
> 他也是HOF
> 
> 并且他接受了一个参数, 在最顶上解构出来了, 一个是 modules , 一个是 nodeOps
> 
> 最上面定义了一个 `cbs`, 这个和`snabbdom`也是类似的, 存储的是模块中定义的钩子函数, 然后通过一个遍历初始化
>  + 首先遍历[hooks](/blogs/vue-resource/virtualDOM/6.html#hooks), hooks内有这几个属性: 'create', 'activate', 'update', 'remove', 'destroy'
>  + 遍历钩子函数的名称, 然后遍历这些名称, 作为cbs的属性, 然后初始化成一个数组
>  + 因为模块有很多, 一个钩子函数可能会对应多个处理函数, 所以是一个数组
>  + 然后遍历modules, 如果modules定义了相关的钩子, 则取出来放到cbs对应的属性数组里面
>  + 最后的形式就是这样: `cbs['update'] = [updateAttrs, updateClass, update...]`, 首先cbs中属性是钩子函数， 值是一个数组, 里面存储钩子函数中提供的响应函数
> 
> 最后返回了 patch 函数
> 
> 在返回patch函数之前, 其实已经初始化好了两个和patch相关的数据, 一个 modules, 一个 nodeOps, 并缓存, 所以说他和snabbdom中的patch是类似的
> 
